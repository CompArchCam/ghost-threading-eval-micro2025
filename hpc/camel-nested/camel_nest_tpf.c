    ...
    for(int x=0; x<MAX_KEY; x+=STEP) {
        for (int y = 0; y < STEP; y++) {
            ...
            if ((x+y) % 14 == 0 || serialize_flag == 1) {
                ...
                if ((x+y) - main_iter_ >= 65) {
                    serialize_flag = 1; 
                } else if ((x+y) - main_iter_ <= 63) {
                    serialize_flag = 0; 
                } else if ((x+y) - main_iter_ <= 0) {
                    serialize_flag = 0; 
                    y = main_iter_ + 10; 
                }
            }
...
;
    ...
    // if (x % 20 == 0)
    #endif 
        atomic_store_explicit(&main_iter, (x+y), memory_order_relaxed); 
        }       
    }
    thpool_wait(thpool); 
