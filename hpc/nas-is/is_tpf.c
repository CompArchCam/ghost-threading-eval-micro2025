...
int histogram_array[NUM_KEYS*MAX_ITERATIONS]; 
size_t count; 
...
void PrefetchThread() {
    ...
    for(int i=0; i < NUM_KEYS; i++) {
        // if (i + 64 < NUM_KEYS)
        //     __builtin_prefetch(&key_buff2[(i+64)]);
        __builtin_prefetch(&key_buff1[key_buff2[i]]); // prefetch target load 
        // *(volatile int*)(&key_buff1[key_buff2[i]]); // target load 
        #ifdef SYNC
        if (serialize_flag)
            __asm__ volatile ("serialize\n\t"); 
        if (i % 80 == 0 || serialize_flag == 1) {
...
#else
    #pragma acc update host(key_buff1,key_buff2)
    thpool_add_work(thpool, (void*) PrefetchThread, NULL); 
    for( i=0; i<NUM_KEYS; i++ ) {
        key_buff1[key_buff2[i]]++; // target load 
        #ifdef SYNC
        if (i % 150 == 0)
