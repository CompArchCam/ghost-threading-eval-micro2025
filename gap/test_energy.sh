...
if [ "$exe_baslines" == "1" ]; then
	...
	out_eng="$out_path/energy-$kernel_name-$graph_name-baseline.txt"
	echo "Baseline: $out_pf."
	perf stat -e power/energy-pkg/ -o $out_eng taskset -c $smt_core0 ./$kernel_name -f $graph -n $repeat > $out_pf 2>&1
	rm $kernel_name
fi 
...
if [ "$exe_homp" == "1" ]; then
	...
	out_eng="$out_path/energy-$kernel_name-$graph_name-homp.txt"
	echo "HOMP: $out_pf."
	perf stat -e power/energy-pkg/ -o $out_eng taskset -c $smt_core0,$smt_core1 ./$kernel_name-omp -f $graph -n $repeat > $out_pf 2>&1
	rm $kernel_name-omp
fi 
...
if [ "$exe_swpf" == "1" ]; then
	...
	out_eng="$out_path/energy-$kernel_name-$graph_name-swpf.txt"
	echo "SWPF: $out_pf."
	perf stat -e power/energy-pkg/ -o $out_eng taskset -c $smt_core0 ./$kernel_name-swpf -f $graph -n $repeat > $out_pf 2>&1
	rm $kernel_name-swpf
fi 
		...
		do
			...
			do
				...
				if [ $un_threshold -le $threshold ]; then
					...
					out_eng="$out_path/energy-$kernel_name_htpf.txt"
					echo "HTPF: $out_pf."
					perf stat -e power/energy-pkg/ -o $out_eng taskset -c $smt_core0,$smt_core1 ./$kernel_tpf_name -f $graph -n $repeat -p $syncfreq -o $threshold -j $skip -q $un_threshold > $out_pf 2>&1
				fi 
				fi
			done 
		done
...
if [ "$exe_best" == "1" ]; then
g++ -std=c++11 -fopenmp -pthread -O3 -Wall -w -DOMP -DNT=2 -DBEST -DTUNING -DHTPF -D$graph_macro src/$kernel_tpf_name.cc -o $kernel_tpf_name
for syncfreq in "${syncfreqs[@]}"
do
	for skip in "${skips[@]}"
	do
